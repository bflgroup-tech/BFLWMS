...
/// <summary>
/// Fires EVERY DAY at 07:00 GST (Arabian Standard Time = UTC+04:00) — one hour
/// after WeeklyVolumeGroupBatchService, so Generate OTS reads the Volume Groups
/// that run refreshed rather than yesterday's snapshot.
///
/// NOTE the class and its JobName still read "Weekly". The cadence changed to
/// daily but JobName='OtsWeekly' keys the existing dbo.WmsRptCountryConfig row,
/// so renaming it would orphan the toggle. Name kept deliberately.
///
...
/// catch-up path makes the offset meaningless — on a restart later in the day
...
/// and simply never records a run for that day, with a warning per wake.
///
    ...
    : BackgroundService
{
    ...
    {
        log.LogInformation("WeeklyOtsBatchService started. Fire: daily 07:00 GST.");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                // Past 07:00 and not yet fired for today's date — covers an app
                // restart that lands mid-window.
                ...
                if (shouldFireNow)
                {
                    log.LogInformation("WeeklyOtsBatchService: firing daily run at {Now}.", nowGst);
    ...
    /// <summary>Returns the next 07:00 GST fire strictly in the future.</summary>
    ...
    private async Task<bool> RunOnceAsync(CancellationToken ct)
    {
        ...
        var (rows, err) = await svc.RunOnceAsync("Daily", "Timer", ct);
